...
class OrderPdf extends PdfGenerator {
	...
	private addProducts(detailedFind: DetailedFind<Order>): OrderPdf {
		const doc = this.doc;
		const unitsStruct = doc.struct("Products");
		...
		for (let i = 0; i < units.length; i++) {
			...
			const owningFolder = ModelFolder.fromInfo(Product.ROLE, this.options.domain.name, this.options.branch.data.name, unit.id || "", this.say);
			...
			unitsStruct.add(
				doc.struct("H1", {}, () => {
					// doc
					// 	.fontSize(18).font("Helvetica-Bold").text(`Product ${unit.displayId} -- ${unitData.title}`);
				})
			);
